...
const server = {
  ...
  saveParam: function(type, index, value, params) {
    ...
    if (JSON.stringify(customImageUpdateMethodConfig) !== '{}') {
      ...
      const pass = method.createHookFunction('sendMessageBefore', d)
      if (!pass) return;
      let msg = pako.gzip(encodeURIComponent(JSON.stringify(d)), {
        to: 'string',
      });
  ...
  openWebSocket: function() {
    ...
    if ('WebSocket' in window) {
      ...
      _this.websocket.onopen = function() {
        ...
        //   const newd = Date.parse(new Date())
        //   console.log('send ping', (newd - lastTime) / 1000)
        //   lastTime = newd
        //           _this.websocket.send("rub");
        //       }, 4000);
      };
      ...
      _this.websocket.onmessage = function(result) {
        ...
        if (message === '用户退出') {
          ...
          Store.cooperativeEdit.changeCollaborationSize = Store.cooperativeEdit.changeCollaborationSize.filter(
            value => {
              return value.id != id;
            },
          );
          Store.cooperativeEdit.checkoutData = Store.cooperativeEdit.checkoutData.filter(
            value => {
              return value.id != id;
            },
          );
  ...
  multipleRangeShow: function(id, name, r, c, value) {
    ...
    if ($('#luckysheet-multipleRange-show-' + id).length > 0) {
      ...
      let itemHtml = `<div
								...
								title="${fullName}"
								style="position: absolute;left: ${col_pre - 1}px;width: ${col -
        col_pre -
        1}px;top: ${row_pre - 1}px;height: ${row -
        row_pre -
        1}px;border: 1px solid ${luckyColor[_this.multipleIndex]};z-index: 15;">

								<div class="username" style="height: 19px;line-height:19px;width: max-content;position: absolute;bottom: ${row -
                  row_pre -
                  1}px;right: 0;background-color: ${
